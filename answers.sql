...
SELECT name, SUM(num_cupcakes) FROM cupcakes LEFT  JOIN orders ON cupcake_id = cupcakes.id GROUP BY name ORDER BY name;  
...
SELECT email FROM customers JOIN orders ON customers.id = customer_id  WHERE orders.num_cupcakes >= 100 ORDER BY REVERSE(email);
...
SELECT fname, lname, email FROM customers JOIN orders ON customers.id = customer_id JOIN cupcakes ON cupcake_id = cupcakes.id  WHERE  cupcakes.name = 'funfetti' AND orders.processed  = 'TRUE' GROUP BY fname, lname,  email;